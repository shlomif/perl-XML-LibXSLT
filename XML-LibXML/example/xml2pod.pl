...
sub handle_class {
  ...
  my $cld = $node->getFirstChild(); 
  while ( $cld ){
    # we'll ignore any other node than Element nodes!
    if ( $cld->getType() == XML_ELEMENT_NODE ){ 
      if ( $cld->getName() eq "short" ) { 
	# here we should find only a text!
